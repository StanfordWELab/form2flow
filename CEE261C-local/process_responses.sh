...
WRITE_IMAGE NAME= ./IMAGES/TOP_AVG_UMAG_Z_${Z_HEIGHT} INTERVAL=100 TARGET 16.9 -9.0 250 CAMERA 16.9 -9.0 809.015 UP 0 -1 0 SIZE 1920 970 WIDTH 408 GEOM PLANE 100 0 ${Z_HEIGHT} 0 0 1 VAR avg(mag(u)) RANGE 0 15.3 COLORMAP GRAYSCALE_RGB"
done

# Generate WRITE_IMAGE commands for each y-plane distance
for Y_DISTANCE in "${Y_PLANES_ARRAY[@]}"; do
    WRITE_IMAGE_COMMANDS="$WRITE_IMAGE_COMMANDS
WRITE_IMAGE NAME= ./IMAGES/TOP_AVG_UMAG_Y_${Y_DISTANCE} INTERVAL=100 TARGET 5.5 0 57 CAMERA 5.5 559 57 UP 0 0 1 SIZE 1920 970 WIDTH 321 GEOM PLANE 0 ${Y_DISTANCE} 0 0 -1 0 VAR avg(mag(u)) RANGE 0 15.3 COLORMAP GRAYSCALE_RGB HIDE_ZONES_NAMED Y1"
done
