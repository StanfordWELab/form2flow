...
WRITE_IMAGE NAME= ./IMAGES/TOP_STD_UMAG_${Z_HEIGHT} INTERVAL=5000 TARGET 0 0 0.6 CAMERA 0 0 10 UP 0 1 0 SIZE 1920 970 WIDTH 3.2 GEOM PLANE -1 0 ${Z_HEIGHT} 0 0 1 VAR rms(mag(u)) RANGE 0 15.3 COLORMAP GRAYSCALE_RGB"
done

# Generate WRITE_IMAGE commands for each y-plane distance
for Y_DISTANCE in "${Y_PLANES_ARRAY[@]}"; do
    WRITE_IMAGE_COMMANDS="$WRITE_IMAGE_COMMANDS
WRITE_IMAGE NAME= ./IMAGES/SIDE_AVG_UMAG_Y_${Y_DISTANCE} INTERVAL=5000 TARGET 0 0 0.4 CAMERA 0 -2.7 0.4 UP 0 0 1 SIZE 1920 970 WIDTH 2.6 GEOM PLANE -1 0 ${Y_DISTANCE} 0 1 0 VAR avg(mag(u)) RANGE 0 15.3 COLORMAP GRAYSCALE_RGB HIDE_ZONES_NAMED X1,Y0,Z1
WRITE_IMAGE NAME= ./IMAGES/SIDE_STD_UMAG_Y_${Y_DISTANCE} INTERVAL=5000 TARGET 0 0 0.4 CAMERA 0 -2.7 0.4 UP 0 0 1 SIZE 1920 970 WIDTH 2.6 GEOM PLANE -1 0 ${Y_DISTANCE} 0 1 0 VAR rms(mag(u)) RANGE 0 15.3 COLORMAP GRAYSCALE_RGB HIDE_ZONES_NAMED X1,Y0,Z1
done
