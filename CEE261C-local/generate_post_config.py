...
def main():
    ...
    pcd_files = glob.glob(os.path.join(probes_dir, "*.pcd"))
    
    # For each .pcd file, extract the plane name and add to the configuration
    for pcd_file in pcd_files:
        filename = os.path.basename(pcd_file)  # e.g., door1.00060000.pcd
        ...
        pxyz_file = os.path.join(probes_dir, f"{name_key}.pxyz")
